...
#define GCKDeviceManagerClass NSClassFromString(@"GCKDeviceManager")
#endif
 ...
 * GCKConnectionSuspendedReason enum.
 */
...
/**
 * A controller for Cast devices. This class can send messages to, receive messages from, launch,
 * and close applications running on a Cast device.
 *
 ...
 * before a connection has been started with @ref connect, or after either the
 * GCKDeviceManagerDelegate::deviceManager:didDisconnectWithError:,
 * GCKDeviceManagerDelegate::deviceManager:didSuspendConnectionWithReason:, or
 * GCKDeviceManagerDelegate::deviceManager:didFailToConnectWithError: delegate callback has been
 * invoked.
 *
 * @deprecated Use GCKSessionManager and GCKCastSession to interact with Cast receivers.
 */
GCK_EXPORT
@interface GCKDeviceManager : NSObject

/**
 * Whether this object will listen for app state notifications. This flag may be specified in the
 * initializer. The default value is <code>NO</code>.
 */
@property(nonatomic, assign, readonly) BOOL ignoreAppStateNotifications;
 ...
 */
@property(nonatomic, assign, readonly) GCKConnectionState applicationConnectionState;

/**
 * True if the device manager has established a connection to the device.
 *
 * @deprecated Use @ref connectionState.
 */
@property(nonatomic, assign, readonly) BOOL isConnected GCK_DEPRECATED("Use connectionState");

/**
 * True if the device manager has established a connection to an application on the device.
 *
 * @deprecated Use @ref applicationConnectionState.
 */
@property(nonatomic, assign, readonly)
    BOOL isConnectedToApp GCK_DEPRECATED("Use applicationConnectionState");

/**
 * True if the device manager is disconnected due to a potentially transient event (for example, the
 * app is backgrounded, or there was a network error which might be solved by reconnecting). Note
 * that the disconnection/connection callbacks will not be called while the device manager attemps
 * to reconnect after a potentially transient event, but the properties will always reflect the
 ...
 */
@property(nonatomic, assign, readwrite) NSTimeInterval reconnectTimeout;

/**
 * The device that is being controlled by this GCKDeviceManager.
 */
@property(nonatomic, assign, readonly) GCKDevice *device;
 ...
 */
@property(nonatomic, copy, readonly, GCK_NULLABLE) NSString *applicationSessionID;
 ...
 * otherwise <code>nil</code>.
 */
@property(nonatomic, copy, readonly, GCK_NULLABLE) NSString *applicationStatusText;
 ...
 *
 * If @ref ignoreAppStateNotifications is <code>NO</code>, the object will listen for
 ...
 *
 * If @ref ignoreAppStateNotifications is <code>YES</code>, the object will not listen for
 ...
 *
 * One of the disconnect methods <b>must</b> be called at some point after @ref connect was called
 * and before this object is released by its owner.
 */
- (void)disconnect;
 ...
 */
- (NSInteger)launchApplication:(NSString *)applicationID
             withLaunchOptions:(GCKLaunchOptions * GCK_NULLABLE_TYPE)launchOptions;

/**
 * Launches an application, optionally relaunching it if it is already running.
 *
 * @deprecated Use @ref launchApplication:withLaunchOptions: instead.
 *
 * @param applicationID The application ID.
 ...
 * @return The request ID, or @ref kGCKInvalidRequestID if the request could not be sent.
 */
- (NSInteger)joinApplication:(NSString * GCK_NULLABLE_TYPE)applicationID;
 ...
 * @return The request ID, or @ref kGCKInvalidRequestID if the request could not be sent.
 */
- (NSInteger)stopApplicationWithSessionID:(NSString * GCK_NULLABLE_TYPE)sessionID;
...
/**
 * The GCKDeviceManager delegate protocol. Unless indicated otherwise, the caller should <b>not</b>
 * release the delegating GCKDeviceManager object from within a delegate method.
 */
GCK_EXPORT
 ...
 *
 * @param deviceManager The device manager.
 * @param error The error that caused the disconnection; <code>nil</code> if there was no error (for
 * example, an intentional disconnect).
 */
- (void)deviceManager:(GCKDeviceManager *)deviceManager
    didDisconnectWithError:(NSError * GCK_NULLABLE_TYPE)error;
 ...
 */
- (void)deviceManager:(GCKDeviceManager *)deviceManager
    didDisconnectFromApplicationWithError:(NSError * GCK_NULLABLE_TYPE)error;
 ...
 */
- (void)deviceManager:(GCKDeviceManager *)deviceManager
    didReceiveApplicationMetadata:(GCKApplicationMetadata * GCK_NULLABLE_TYPE)metadata;
 ...
 */
- (void)deviceManager:(GCKDeviceManager *)deviceManager
    didReceiveApplicationStatusText:(NSString * GCK_NULLABLE_TYPE)applicationStatusText;
